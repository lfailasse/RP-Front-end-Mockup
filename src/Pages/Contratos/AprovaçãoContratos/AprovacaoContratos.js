...
export const AprovacaoContratos = () => {
    ...
    async function getAllRP() {
        setLoading(true)
        await axiosInstance.get("/rp/contratos/aprovacao", { headers: headers }).then((res) => setRPs(res.data))
        setLoading(false)
    }
    ...
    async function aprovarRP(id) {
        if (window.confirm(`Deseja aprovar esta RP? (${id})`)) {
            await axiosInstance.patch(`/rp/contratos/approve/${id}`, {}, { headers: headers })
            getAllRP()
        }
    }
    ...
    return (
        <div style={{ pointerEvents: LoadingAnimation ? 'none' : 'auto' }}>
            <div>
                <Head title="Aprovação de RPs - Contratos"></Head>
            </div>
