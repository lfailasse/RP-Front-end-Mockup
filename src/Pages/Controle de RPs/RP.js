...
import { IoIosEye, IoIosEyeOff, IoIosArrowBack, IoIosArrowForward, IoIosInformationCircleOutline, IoIosSave, IoMdArrowDropdown, IoMdArrowDropup, IoMdArrowBack } from 'react-icons/io'
...
export const RP = () => {
    ...
    async function searchrpwfilter() {
        if (Filtered === true) {
            try {
                setLoading(true)
                await axiosInstance.get(`/rp/search?filterType=${Filtertype}&filterSearch=${Search.toUpperCase()}&page=${CPage}&days=${Period}&startDate=${StartDate}&endDate=${EndDate}&filter=${CFilter}&department=&role=${userData.role}&sortColumn=${Column}&order=${Order}`, { headers: headers }).then((res) => setRPs(res.data))
                setFiltered(true)
            } catch (e) {
                console.log(e)
            }
            ...
            try {
                setLoading(true)
                await axiosInstance.get(`/rp/search?filterType=${Filtertype}&filterSearch=${Search.toUpperCase()}&page=${0}&days=${Period}&startDate=${StartDate}&endDate=${EndDate}&filter=${CFilter}&department=&role=${userData.role}&sortColumn=${Column}&order=${Order}`, { headers: headers }).then((res) => setRPs(res.data))
                setFiltered(true)
            } catch (e) {
                console.log(e)
            }
    ...
    function editDataRP(e) {
        ...
        if (e.target.id === "projeto") {
            ...
            const classefinanceira = currentRubrica?.classefinanceira?.find(cfinanceira => cfinanceira.id === parseInt(e.target.value)) || ""
            newRP["classefinanceira"] = classefinanceira
            setSelectedCFinanceira(classefinanceira)
        } else {
            const centrodecustos = currentProject?.centrodecustos?.find(cenc => cenc.id === parseInt(e.target.value)) || ""
    ...
    async function sendEditRP(e, id) {
        ...
        if (window.confirm(`Deseja atualizar esta RP? (${id})`)) {
            ...
            await axiosInstance.patch(`/rp/statusok/${id}`, {}, { headers: headers })
            window.location.reload()
        }
    }
                    ...
                    <div>
                        ...
                        <div className='div_main_columns'>
                            <div className='div_columns'>
                                <div className='div_column'><label id='count' onClick={(e) => sortColumn(e.target.id)}>SETOR</label>{Column == "count" && Order ? <IoMdArrowDropdown /> : Column == "count" && Order == false ? <IoMdArrowDropup /> : null}</div>
                                <div className='div_column'><label id='dataemissao' onClick={(e) => sortColumn(e.target.id)}>NÚMERO DA RP</label>{Column == "dataemissao" && Order ? <IoMdArrowDropdown /> : Column == "dataemissao" && Order == false ? <IoMdArrowDropup /> : null}</div>
                                ...
                                <div className='div_column'><label id='centrodecustos' onClick={(e) => sortColumn(e.target.id)}>SETOR</label>{Column == "setor" && Order ? <IoMdArrowDropdown /> : Column == "setor" && Order == false ? <IoMdArrowDropup /> : null}</div>
                            ...
                            {Loading ? <div className='div_main_info_columns'><label className='div_info_columns_loading_label'>Carregando...</label></div> : RPs?.content?.map((rp, index) => (
                                <div className='div_main_info_column' key={index}>
                                    <div className={isMobile ? rp.status && rp.rejected && view[index] ? 'div_info_columns_rejected_opened' : rp.status && rp.rejected ? 'div_info_columns_rejected' : rp.status && view[index] ? "div_info_columns_canceled_opened" : rp.status ? "div_info_columns_canceled" : view[index] ? "div_info_columns_opened" : 'div_info_columns' : 'div_info_columns'}>
                                        <div className='div_info_column' onClick={isMobile ? () => { setRPID(rp.id); viewpdf(index); getSignature1(rp.rp_signature1); getSignature2(rp.rp_signature2); getSignatureContratos(rp.rp_signature_contratos) } : null} style={{ color: rp.status && rp.rejected ? "orange" : rp.status ? "red" : "white" }}>
                                            <div className='div_info_column_content' style={{ color: rp.rejected ? 'orange' : rp.status ? 'red' : rp.financeirostatus === "Recebido" ? '#00FFE1' : rp.aprovacaoOrcamento ? '#00FF00' : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? 'white' : 'yellow' }}><label className='label_content_title'>Setor: </label><label>{rp.department}</label></div>
                                            <div className='div_info_column_content' style={{ color: rp.rejected ? 'orange' : rp.status ? 'red' : rp.financeirostatus === "Recebido" ? '#00FFE1' : rp.aprovacaoOrcamento ? '#00FF00' : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? 'white' : 'yellow' }}><label className='label_content_title'>Número da RP: </label><label>{rp.id}</label></div>
                                            <div className='div_info_column_content' style={{ color: rp.rejected ? 'orange' : rp.status ? 'red' : rp.financeirostatus === "Recebido" ? '#00FFE1' : rp.aprovacaoOrcamento ? '#00FF00' : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? 'white' : 'yellow' }}><label className='label_content_title'>Data de emissão: </label><label>{rp.dataemissao}</label></div>
                                            <div className='div_info_column_content' style={{ color: rp.rejected ? 'orange' : rp.status ? 'red' : rp.financeirostatus === "Recebido" ? '#00FFE1' : rp.aprovacaoOrcamento ? '#00FF00' : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? 'white' : 'yellow' }}><label className='label_content_title'>Data de pagamento: </label><label>{rp.datapagamento}</label></div>
                                            <Tooltip title={rp.favorecido} placement='top'><div className='div_info_column_content' style={{ color: rp.rejected ? 'orange' : rp.status ? 'red' : rp.financeirostatus === "Recebido" ? '#00FFE1' : rp.aprovacaoOrcamento ? '#00FF00' : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? 'white' : 'yellow' }}><label className='label_content_title'>Favorecido: </label><label>{rp.favorecido ? rp.favorecido : "SEM FAVORECIDO"}</label></div></Tooltip>
                                            <div className='div_info_column_content' style={{ color: rp.rejected ? 'orange' : rp.status ? 'red' : rp.financeirostatus === "Recebido" ? '#00FFE1' : rp.aprovacaoOrcamento ? '#00FF00' : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? 'white' : 'yellow' }}><label className='label_content_title'>Projeto: </label><label>{rp.projeto?.name}</label></div>
                                            <Tooltip title={rp.rubrica?.code + ' - ' + rp.rubrica?.name} placement='top'><div className='div_info_column_content' style={{ color: rp.rejected ? 'orange' : rp.status ? 'red' : rp.financeirostatus === "Recebido" ? '#00FFE1' : rp.aprovacaoOrcamento ? '#00FF00' : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? 'white' : 'yellow' }}><label className='label_content_title'>Rubrica: </label><label>{rp.rubrica ? rp.rubrica?.code + ' - ' + rp.rubrica?.name : "Indefinido"}</label></div></Tooltip>
                                            <Tooltip title={rp.classefinanceira?.code + ' - ' + rp.classefinanceira?.name} placement='top'><div className='div_info_column_content' style={{ color: rp.rejected ? 'orange' : rp.status ? 'red' : rp.financeirostatus === "Recebido" ? '#00FFE1' : rp.aprovacaoOrcamento ? '#00FF00' : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? 'white' : 'yellow' }}><label className='label_content_title'>Rubrica: </label><label>{rp.classefinanceira ? rp.classefinanceira?.code + ' - ' + rp.classefinanceira?.name : "Indefinido"}</label></div></Tooltip>
                                            <div className='div_info_column_content' style={{ color: rp.rejected ? 'orange' : rp.status ? 'red' : rp.financeirostatus === "Recebido" ? '#00FFE1' : rp.aprovacaoOrcamento ? '#00FF00' : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? 'white' : 'yellow' }}><label className='label_content_title'>Valor: </label><label>{rp.total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</label></div>
                                            <div className='div_info_column_content_visualizar'>
                                                <Tooltip title={rp.rejected ? "Status: Devolvida" : rp.status ? "Status: Cancelada" : rp.financeirostatus === "Recebido" ? "Status: Recebida pelo financeiro" : rp.aprovacaoOrcamento ? "Status: Aguardando recebimento do financeiro" : (rp.projetoEmitido !== "MATRIZ" && !rp.aprovacaoGerenteMatriz) || rp.projetoEmitido === "MATRIZ" && (!rp.aprovacaoCompras && !rp.aprovacaoAdministrativo && !rp.aprovacaorh) ? "Status: Aguardando o envio da requisição por parte do setor" : "Status: Aguardando classificação do Orçamento"} placement='top'>
                                                ...
                                                <button className='button_visualizarpdf' value={"Ver PDF"} onClick={() => { setRPID(rp.id); viewpdf(index); getSignature1(rp.rp_signature1); getSignature2(rp.rp_signature2); getSignatureContratos(rp.rp_signature_contratos) }}>{view[index] ? "Ocultar PDF" : "Ver PDF"}</button>
                                            </div>
                                            {rp.status ? <div className='button_cancelarpdf_false'></div> : <div onClick={() => cancelRP(rp.id)} className='button_cancelarpdf_true' style={{ right: view[index] ? 60 : 70 }}><MdBlock className='pdf_cancel' size={20}></MdBlock></div>}
                                                        ...
                                                        <div className='modal_editrp_rpitems'>
                                                            ...
                                                            {editRP?.rpitems?.map((item, index) => (
                                                                <div className='div_editrp_rpitems'>
                                                                    <div key={editRP?.id} className='div_editrp_rp_atributo'><input id='name' onChange={(e) => editDataRPItems(e, index)} className='div_editrp_rpitems_atributo_inputname' defaultValue={item.name}></input><input id='valor' onChange={(e) => editDataRPItems(e, index)} className='div_editrp_rp_atributo_inputvalue' type={'number'} step={0.01} defaultValue={item.valor}></input></div>
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </div>
