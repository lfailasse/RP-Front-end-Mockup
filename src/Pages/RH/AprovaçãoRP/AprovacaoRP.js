...
export const AprovacaoRP = () => {
    ...
    async function getAllRP() {
        setLoading(true)
        await axiosInstance.get("/rp/rh/aprovacao", { headers: headers }).then((res) => setRPs(res.data))
        setLoading(false)
    }
    ...
    return (
        <div style={{ pointerEvents: LoadingAnimation ? 'none' : 'auto' }}>
            <div>
                <Head title="Aprovação de RPs - RH"></Head>
            </div>
