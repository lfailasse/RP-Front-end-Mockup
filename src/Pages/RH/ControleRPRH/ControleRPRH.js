...
import { IoIosArrowBack, IoIosEye, IoIosEyeOff, IoIosArrowForward, IoIosInformationCircleOutline, IoIosSave, IoMdArrowDropdown, IoMdArrowDropup, IoMdArrowBack } from 'react-icons/io'
...
export const ControleRPRH = () => {
    ...
    async function searchrpwfilter() {
        if (Filtered === true) {
            try {
                setLoading(true)
                await axiosInstance.get(`/rp/search?filterType=${Filtertype}&filterSearch=${Search.toUpperCase()}&page=${CPage}&days=${Period}&startDate=${StartDate}&endDate=${EndDate}&filter=${CFilter}&department=RH&role=${userData.role}&sortColumn=${Column}&order=${Order}`, { headers: headers }).then((res) => setRPs(res.data))
                setFiltered(true)
            } catch (e) {
                console.log(e)
            }
            ...
            try {
                setLoading(true)
                await axiosInstance.get(`/rp/search?filterType=${Filtertype}&filterSearch=${Search.toUpperCase()}&page=${0}&days=${Period}&startDate=${StartDate}&endDate=${EndDate}&filter=${CFilter}&department=RH&role=${userData.role}&sortColumn=${Column}&order=${Order}`, { headers: headers }).then((res) => setRPs(res.data))
                setFiltered(true)
            } catch (e) {
                console.log(e)
            }
                                    ...
                                    <div className='div_main_info_column' key={index}>
                                        <div className={isMobile ? rp.status && rp.rejected && view[index] ? 'div_info_columns_rejected_opened' : rp.status && rp.rejected ? 'div_info_columns_rejected' : rp.status && view[index] ? "div_info_columns_canceled_opened" : rp.status ? "div_info_columns_canceled" : view[index] ? "div_info_columns_opened" : 'div_info_columns' : 'div_info_columns'}>
                                            <div className='div_info_column' onClick={isMobile ? () => { setRPID(rp.id); viewpdf(index); getSignature1(rp.rp_signature1); getSignature2(rp.rp_signature2); getSignatureContratos(rp.rp_signature_contratos) } : null} style={{ color: rp.status && rp.rejected ? "orange" : rp.status ? "red" : "white" }}>
                                                ...
                                                <Tooltip title={rp.rubrica?.code + ' - ' + rp.rubrica?.name} placement='top'><div className='div_info_column_content'><label className='label_content_title'>Rubrica: </label><label>{rp.rubrica?.code + ' - ' + rp.rubrica?.name}</label></div></Tooltip>
                                                <Tooltip title={rp.classefinanceira?.code + ' - ' + rp.classefinanceira?.name} placement='top'><div className='div_info_column_content'><label className='label_content_title'>Rubrica: </label><label>{rp.classefinanceira?.code + ' - ' + rp.classefinanceira?.name}</label></div></Tooltip>
                                                ...
                                                <div className='div_info_column_content_visualizar'>
                                                    <Tooltip title={rp.status ? "Status: Cancelada" : !rp.aprovacaoRH ? "Status: Aguardando aprovação do RH" : !rp.aprovacaoOrcamento ? "Status: Aguardando aprovação do Orçamento" : rp.financeirostatus !== "Pago" ? "Status: Aguardando pagamento" : "Status: Pago"} placement='top'>
                                                ...
                                                {rp.rp_signature1 ? null : <div onClick={() => rpEditOpen(rp.id)} className='button_editrp' style={{ left: 50 }}><FaPencilAlt className='rp_edit' size={15}></FaPencilAlt></div>}
                                            </div>
                                        </div>
                                        {view[index] ?
