...
export const AprovacaoCompras = () => {
    ...
    async function getAllRP() {
        setLoading(true)
        await axiosInstance.get("/rp/compras/aprovacao", { headers: headers }).then((res) => setRPs(res.data))
        setLoading(false)
    }
    ...
    async function aprovarRP(id) {
        if (window.confirm(`Deseja aprovar esta RP? (${id})`)) {
            await axiosInstance.patch(`/rp/compras/approve/${id}`, {}, { headers: headers })
            setView([false])
            getAllRP()
        }
    }
    ...
    return (
        <div style={{ pointerEvents: LoadingAnimation ? 'none' : 'auto' }}>
            <div>
                <Head title="Aprovação de RPs - Compras"></Head>
            </div>
                    ...
                    <div style={{ width: '90%' }} key={index}>
                        <div style={{ borderTop: '1px solid white', display: "flex", flexDirection: "row", justifyContent: "center", textAlign: "left" }}>
                            <div style={{ width: '100%', display: "flex", flexDirection: "row", justifyContent: 'center', textAlign: 'center', gap: '20px', color: rp.status && rp.rejected ? "orange" : rp.status ? "red" : "white" }}>
                                ...
                                <Tooltip title={rp.classefinanceira?.code + ' - ' + rp.classefinanceira?.name} placement='top'><div style={{ width: '11%', textOverflow: 'ellipsis', whiteSpace: 'nowrap', overflow: 'hidden' }}><label>{rp.rubrica ? rp.classefinanceira?.code + ' - ' + rp.classefinanceira?.name : "Indefinido"}</label></div></Tooltip>
                                ...
                                <div style={{ width: '11%', display: "flex", flexDirection: "row", justifyContent: "center", alignItems: 'center', flexWrap: 'nowrap' }}>
                                    <button style={{ marginTop: '4px' }} value={"Ver PDF"} onClick={async () => { setRPID(rp.id); await getSignature1(rp.rp_signature1); await getSignature2(rp.rp_signature2); viewpdf(index) }}>{view[index] ? "Ocultar PDF" : "Ver PDF"}</button>
                                </div>
                            </div>
                        </div>
                        {view[index] ?
                            <div className='div_pdfview_info'>
                                ...
                                {userData.role === "COORDENADOR COMPRAS" ?
                                    <>
