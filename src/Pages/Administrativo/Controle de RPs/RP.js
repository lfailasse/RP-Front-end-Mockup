...
import { IoIosArrowBack, IoIosArrowForward, IoIosInformationCircleOutline, IoIosSave, IoMdArrowDropdown, IoMdArrowDropup, IoMdArrowBack, IoIosEyeOff, IoIosEye } from 'react-icons/io'
...
export const RP = () => {
    ...
    async function searchrpwfilter() {
        if (Filtered === true) {
            try {
                setLoading(true)
                await axiosInstance.get(`/rp/search?filterType=${Filtertype}&filterSearch=${Search.toUpperCase()}&page=${CPage}&days=${Period}&startDate=${StartDate}&endDate=${EndDate}&filter=${CFilter}&department=ADMINISTRATIVO&role=${userData.role}&sortColumn=${Column}&order=${Order}`, { headers: headers }).then((res) => setRPs(res.data))
                setFiltered(true)
            } catch (e) {
                console.log(e)
            }
            ...
            try {
                setLoading(true)
                await axiosInstance.get(`/rp/search?filterType=${Filtertype}&filterSearch=${Search.toUpperCase()}&page=${0}&days=${Period}&startDate=${StartDate}&endDate=${EndDate}&filter=${CFilter}&department=ADMINISTRATIVO&role=${userData.role}&sortColumn=${Column}&order=${Order}`, { headers: headers }).then((res) => setRPs(res.data))
                setFiltered(true)
            } catch (e) {
                console.log(e)
            }
                                        ...
                                        <div className={isMobile ? rp.status && rp.rejected && view[index] ? 'div_info_columns_rejected_opened' : rp.status && rp.rejected ? 'div_info_columns_rejected' : rp.status && view[index] ? "div_info_columns_canceled_opened" : rp.status ? "div_info_columns_canceled" : view[index] ? "div_info_columns_opened" : 'div_info_columns' : 'div_info_columns'}>
                                            <div className='div_info_column' onClick={isMobile ? () => { setRPID(rp.id); viewpdf(index); getSignature1(rp.rp_signature1); getSignature2(rp.rp_signature2); getSignatureContratos(rp.rp_signature_contratos) } : null} style={{ color: rp.status && rp.rejected ? "orange" : rp.status ? "red" : "white" }}>
                                                ...
                                                <div className='div_info_column_content_visualizar'>
                                                    <Tooltip title={rp.status ? "Status: Cancelada" : !rp.aprovacaoAdministrativo ? "Status: Aguardando aprovação do Administrativo" : !rp.aprovacaoOrcamento ? "Status: Aguardando aprovação do Orçamento" : rp.financeirostatus !== "Pago" ? "Status: Aguardando pagamento" : "Status: Pago"} placement='top'>
